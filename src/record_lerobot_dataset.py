...
from lerobot.tests.mocks.mock_robot import MockRobotConfig  # noqa: F401
from lerobot.tests.mocks.mock_teleop import MockTeleopConfig  # noqa: F401
...
# Circle annotation configuration - easily adjustable
CIRCLE_RADIUS = 39  # 30% bigger than original 30
CIRCLE_THICKNESS = 4
...
def annotate_circles_popup(image: np.ndarray) -> list[tuple[int, int]]:
    """
    Show a popup window for the user to place two hollow circles by clicking.

    Args:
        image: RGB numpy array image to annotate

    Returns:
        List of (x, y) coordinates for the two circles
    """
    ...
    circles = []
    current_preview_pos = None
    should_close = False

    def mouse_callback(event, x, y, flags, param):
        nonlocal circles, current_preview_pos, display_image, should_close

        if event == cv2.EVENT_MOUSEMOVE:
            # Show preview circle at mouse position
            current_preview_pos = (x, y)
            display_image = original_image.copy()

            # Draw already placed circles
            for i, (circle_x, circle_y) in enumerate(circles):
                cv2.circle(display_image, (circle_x, circle_y), CIRCLE_RADIUS, CIRCLE_COLORS[i], CIRCLE_THICKNESS)
                # Add center dot with same color as circle
                cv2.circle(display_image, (circle_x, circle_y), CENTER_DOT_RADIUS, CIRCLE_COLORS[i], -1)

            # Draw preview circle if we haven't placed both circles yet
            if len(circles) < 2:
                # Use the color of the next circle to be placed
                next_color = CIRCLE_COLORS[len(circles)]
                cv2.circle(display_image, (x, y), CIRCLE_RADIUS, next_color, CIRCLE_THICKNESS)
                # Add preview center dot with same color as circle
                cv2.circle(display_image, (x, y), CENTER_DOT_RADIUS, next_color, -1)
            ...
            if len(circles) < 2:
                ...
                # Draw all placed circles with their correct colors
                for i, (circle_x, circle_y) in enumerate(circles):
                    cv2.circle(display_image, (circle_x, circle_y), CIRCLE_RADIUS, CIRCLE_COLORS[i], CIRCLE_THICKNESS)
                    # Add center dot with same color as circle
                    cv2.circle(display_image, (circle_x, circle_y), CENTER_DOT_RADIUS, CIRCLE_COLORS[i], -1)

                print(f"Circle {len(circles)} placed at ({x}, {y})")

                if len(circles) == 2:
                    print("Both circles placed! Press any key to finish or 'r' to reset, or click anywhere to close.")
            else:
                # Third click anywhere - close the window
                print("Closing annotation window.")
                should_close = True

    # Create window and set mouse callback
    cv2.namedWindow(
        'Click to place 2 circles (Press any key to finish, "r" to reset, 3rd click to close)', cv2.WINDOW_AUTOSIZE
    )
    cv2.setMouseCallback(
        'Click to place 2 circles (Press any key to finish, "r" to reset, 3rd click to close)', mouse_callback
    )

    print(
        "Click on the image to place 2 hollow circles. Press any key to finish at any time, 'r' to reset, or click anywhere after placing 2 circles to close."
    )

    while True:
        cv2.imshow(
            'Click to place 2 circles (Press any key to finish, "r" to reset, 3rd click to close)', display_image
        )
        key = cv2.waitKey(1) & 0xFF

        # Check if we should close from mouse callback
        if should_close:
            break
        ...
        if key == ord("r"):
            ...
            circles = []
            display_image = original_image.copy()
            print("Circles reset. Click to place new circles.")

        elif key != 255 and key != 0:  # Any key pressed (excluding no key)
            print(f"Finishing annotation with {len(circles)} circle(s) placed.")
            break
    ...
    # Give OpenCV time to properly close the window
    cv2.waitKey(1)
    return circles
...
def record_loop(
    ...
    display_data: bool = False,
    circle_coords: list[tuple[int, int]] | None = None,
):
    ...
    while control_time_s is not None and timestamp < control_time_s:
        ...
        # Overlay circles on context image if coordinates are provided
        if circle_coords and len(circle_coords) == 2 and "context" in observation:
            context_image = observation["context"].copy()
            for i, (x, y) in enumerate(circle_coords):
                # Convert BGR color to RGB for the recording (since observation["context"] is in RGB)
                bgr_color = CIRCLE_COLORS[i]
                rgb_color = (bgr_color[2], bgr_color[1], bgr_color[0])  # BGR → RGB
                cv2.circle(context_image, (x, y), CIRCLE_RADIUS, rgb_color, CIRCLE_THICKNESS)

                # Add center dot (convert BGR to RGB)
                bgr_dot_color = CIRCLE_COLORS[i]
                rgb_dot_color = (bgr_dot_color[2], bgr_dot_color[1], bgr_dot_color[0])  # BGR → RGB
                cv2.circle(context_image, (x, y), CENTER_DOT_RADIUS, rgb_dot_color, -1)
            observation["context"] = context_image
